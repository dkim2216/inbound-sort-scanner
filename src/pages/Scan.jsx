...
export default function Scan({ sessionId, sessions }) {
    ...
    setDestinations((prev) =>
      prev.map((dest) =>
        dest.id === id
          ? {
              ...dest,
              actual_qty: value === '' ? '' : Math.max(0, Number.parseInt(value, 10) || 0)
            }
          : dest
      )
    );
  ...
  const handleMarkDone = async (id) => {
    ...
    const actualQty =
      dest.actual_qty === '' ? NaN : Number.parseInt(dest.actual_qty, 10);
    ...
    if (discrepancy !== 0 && !String(dest.remark || '').trim()) {
      setMessage({
        type: 'error',
        text: 'Please add a remark when there is a discrepancy'
      });
      return;
    }
    ...
    try {
      ...
      setMessage({
        ...
        text:
          discrepancy === 0
            ? 'Marked as sorted!'
            : 'Saved with discrepancy and remark!'
      });
    } catch (err) {
  ...
  return (
    <div className="p-4 md:p-8 max-w-6xl mx-auto">
      <div className="mb-6">
        <h2 className="text-2xl md:text-3xl font-bold" style={{ color: CS_NAVY }}>
          Scan Workflow
        </h2>
      ...
      {step === 1 && (
        <div className="bg-white rounded-2xl border border-gray-100 p-4 md:p-8 shadow-sm">
          <h3 className="text-lg font-semibold mb-5" style={{ color: CS_NAVY }}>
            Step 1: Select Case ID
          </h3>
              ...
              {cases.map((caseId) => {
                ...
                return (
                  <button
                    ...
                    onMouseEnter={(e) => {
                      if (status === 'pending') e.currentTarget.style.borderColor = CS_TEAL;
                    }}
                    onMouseLeave={(e) => {
                      if (status === 'pending') e.currentTarget.style.borderColor = '#E5E7EB';
                    }}
                  >
                    <div className="text-base font-bold" style={{ color: CS_NAVY }}>
                      {caseId}
                    </div>
                    {progress && (
            ...
            {skus.map((item) => {
              ...
              return (
                <button
                  ...
                  disabled={loading}
                  className={`p-4 border-2 rounded-xl font-medium disabled:opacity-50 transition-all ${
                    status === 'completed'
                      ? 'bg-green-50 border-green-400'
                      : status === 'in-progress'
                      ? 'bg-yellow-50 border-yellow-400'
                ...
                >
                  <div className="font-bold" style={{ color: CS_NAVY }}>
                    {item.sku}
                  </div>
                  <div className="text-xs text-gray-400 mt-1">
      ...
      {step === 3 && (
        <div className="bg-white rounded-2xl border border-gray-100 p-4 md:p-8 shadow-sm">
          <BackBar label="Step 3: Confirm Qty / Remark / Mark Done" />
                  ...
                  <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          <p className="font-bold text-lg md:text-xl" style={{ color: CS_NAVY }}>
                            {dest.dealer}
                          </p>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-5">
                        <div>
                          ...
                          <p className="font-bold text-lg mt-0.5" style={{ color: CS_TEAL }}>
                            {dest.sort_group}
                          </p>
                        </div>
                      ...
                      <div className="mt-4">
                        ...
                        <textarea
                          ...
                          placeholder={
                            hasDiscrepancy
                              ? 'Explain shortage / overage / issue...'
                              : 'Optional remark'
                          }
                        ...
                        {hasDiscrepancy && !String(dest.remark || '').trim() && (
                          <p className="text-xs text-amber-600 mt-2">
                            Remark is required when discrepancy exists.
                          </p>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="lg:w-56 flex flex-col gap-3">
                      {dest.done && (
                        <div
                          className={`flex items-center justify-center gap-2 font-semibold rounded-xl px-4 py-2.5 ${
                            hasDiscrepancy
                      ...
                      <button
                        ...
                        disabled={isSaving}
                        className="w-full flex items-center justify-center gap-2 text-white px-5 py-2.5 rounded-xl font-semibold transition-opacity disabled:opacity-50"
